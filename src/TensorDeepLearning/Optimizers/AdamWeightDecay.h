#ifndef ADAM_WEIGHT_DECAY_H
#define ADAM_WEIGHT_DECAY_H
...
template <typename T> class AdamWeightDecay : public Optimizer<T> {
public:
  ...
  AdamWeightDecay(ArenaArray<ArenaTensorNode<T> *> OptimizationParameters,
                  Scheduler<T> *LearningRateScheduler, T MomentumFactor,
                  T SecondMomentFactor, T WeightDecayFactor, T Epsilon)
      : Optimizer<T>::Optimizer(OptimizationParameters, LearningRateScheduler),
        ...
        WeightDecayFactor(WeightDecayFactor), Epsilon(Epsilon) {
